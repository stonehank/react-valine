...
describe('Test InputContainer List', ()=>{
  ...
  it('choose by click', () => {
    ...
    expect(wrapper.find(".vemoji-preview-list").length).toBe(0)
    expect(wrapper.find('textarea').at(0).text()).toBe("💯 ")
  });
  ...
  it('choose by keyboard', () => {
    ...
    wrapper.find('textarea').at(0).simulate('keydown',{keyCode:38,preventDefault:()=>{}})
    expect(wrapper.find(".vemoji-preview-list").length).toBe(1)
    wrapper.find('textarea').at(0).simulate('keydown',{keyCode:13,preventDefault:()=>{}})
    expect(wrapper.find(".vemoji-preview-list").length).toBe(0)
    expect(wrapper.find('textarea').at(0).text()).toBe("✔️ ")
  });
  ...
  it("emojiPreview cancel by keyboard",()=>{
    ...
    expect(wrapper.find(".vemoji-preview-list").length).toBe(1)
    wrapper.find('textarea').at(0).simulate('focus')
    wrapper.find('textarea').at(0).simulate('keydown',{keyCode:27,preventDefault:()=>{}})
    expect(wrapper.find(".vemoji-preview-list").length).toBe(0)
    expect(wrapper.find('textarea').at(0).text()).toBe(":")
  })
