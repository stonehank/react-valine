...
const test_uniq_str="test-multi-nest-comments"
...
nock('https://i5daxohp.api.lncld.net:443')
  .persist()
  .get('/1.1/classes/Comment?where=%7B%22uniqStr%22%3A%22test-multi-nest-comments%22%2C%22pid%22%3A%22%22%7D&keys=nick%2Ccomment%2Clink%2Cpid%2CavatarSrc%2Crid&limit=10&order=-createdAt')
...
nock('https://i5daxohp.api.lncld.net:443')
  .persist()
  // .log(console.log)
  .get("/1.1/classes/Comment?where=%7B%22uniqStr%22%3A%22test-multi-nest-comments%22%2C%22pid%22%3A%22%22%7D&keys=nick%2Ccomment%2Clink%2Cpid%2CavatarSrc%2Crid&limit=10&skip=10&order=-createdAt")
...
nock('https://i5daxohp.api.lncld.net')
  // .log(console.log)
  .persist()
  .get("/1.1/classes/Comment?where=%7B%22uniqStr%22%3A%22test-multi-nest-comments%22%2C%22pid%22%3A%7B%22%24ne%22%3A%22%22%7D%2C%22rid%22%3A%7B%22%24in%22%3A%5B%225ceffaa030863b00686fa39b%22%2C%225cefee85ba39c80068ad7da9%22%2C%225cee8a1d43e78c006734fc8e%22%5D%7D%7D&order=createdAt")
...
nock('https://i5daxohp.api.lncld.net')
  .persist()
  // .log(console.log)
  .get("/1.1/classes/Comment?where=%7B%22uniqStr%22%3A%22test-multi-nest-comments%22%2C%22pid%22%3A%22%22%7D&keys=nick%2Ccomment%2Clink%2Cpid%2CavatarSrc%2Crid&limit=10&skip=13&order=-createdAt")
  .reply(200, {"results":[]})
...
nock('https://i5daxohp.api.lncld.net')
  .persist()
  .get('/1.1/classes/Comment?where=%7B%22uniqStr%22%3A%22test-multi-nest-comments%22%7D&limit=0&count=1')
...
nock("https://i5daxohp.api.lncld.net")
  .persist()
  .get("/1.1/classes/Counter?where=%7B%22uniqStr%22%3A%22test-multi-nest-comments%22%7D")
  .reply(200, {"results":[{"uniqStr":"test-multi-nest-comments","title":"\u6d4b\u8bd5\u9875\u9762localhost","time":9999,"createdAt":"2019-05-29T14:53:57.872Z","updatedAt":"2019-05-30T08:08:47.209Z","objectId":"5cee9d0530863b006861c98c"}]})
...
describe('test nest with multi comments', ()=>{
  ...
  it('fetch more',(done)=>{
    ...
    setTimeout(()=>{
      expect(list[0].childNodes.length).toBe(13)
      ...
      setTimeout(()=>{
        ...
        expect(errlog[0].innerHTML).toBe("被 @ 的回复消失了，嵌套模式无法查看！")
        done()
      },1000)
    },1000)
  })
  ...
  it("click check reply",()=>{
    ...
    expect(checkReplyBtn.length).toBe(5)
    TestUtil.Simulate.click(checkReplyBtn[0])
    expect(checkReplyBtn.length).toBe(4)
